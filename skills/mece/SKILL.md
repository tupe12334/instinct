...
SPLIT LOGIC: [the dimension used to divide — e.g., "revenue = volume x price x mix"]

LEVEL 1 BUCKETS
├── [Bucket A] — [scope definition: what's included / excluded]
├── [Bucket B] — [scope definition]
└── [Bucket C] — [scope definition]

OVERLAP CHECK
- A vs B: [no overlap because ...]
- A vs C: [no overlap because ...]
- B vs C: [no overlap because ...]

GAP CHECK
- Scenario tested: [edge case 1] → fits in [Bucket X]
- Scenario tested: [edge case 2] → fits in [Bucket Y]
- Conclusion: no gaps identified / [gap found: add Bucket D]

LEVEL 2 BREAKDOWN (for Bucket A)
├── [A1] — [definition]
└── [A2] — [definition]
```
